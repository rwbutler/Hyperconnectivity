...
class ConfigurationTests: XCTestCase {
    ...
    func testShouldCheckConnectivityByDefault() {
        let sut = Configuration()
        XCTAssertTrue(sut.shouldCheckConnectivity)
    }
    
    func testDefaultSuccessThresholdIs50Percent() {
        let sut = Configuration()
        XCTAssertEqual(sut.successThreshold, Percentage(50.0))
    }
    ...
    func testCloneForReachabilityProducesAClone() {
        let sut = Configuration()
        XCTAssertFalse(sut.cloneForReachability() === sut)
    }
    
    func testReachabilityCloneDoesntCheckConnectivity() {
        let sut = Configuration()
        XCTAssertFalse(sut.cloneForReachability().shouldCheckConnectivity)
    }
    
    func testReachabilityCloneSuccessThresholdIsZero() {
        let sut = Configuration()
        XCTAssertEqual(sut.cloneForReachability().successThreshold, Percentage.init(0.0))
    }
    
    func testReachabilityCloneConnectivityURLsAreEmpty() {
        let sut = Configuration()
        XCTAssertTrue(sut.cloneForReachability().connectivityURLs.isEmpty)
    }
